...
function renderDecisionMessage(result) {
  ...
  if (result.reason === "all_unavailable_or_exhausted") {
...
function formatSwitchingBanner() {
  return colorize(ANSI_BOLD_GREEN, "[CDX] SWITCHING ACCOUNT...");
}

function formatDecisionBanner(result) {
  return colorize(ANSI_BOLD_GREEN, `[CDX] ${renderDecisionMessage(result)}`);
}

function formatFailureBanner(message) {
  return colorize(ANSI_BOLD_GREEN, `[CDX] ${String(message || "Smart switch failed.")}`);
}

function formatInfoBanner(message) {
  return colorize(ANSI_BOLD_GREEN, `[CDX] ${String(message || "")}`);
}
