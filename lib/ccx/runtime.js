...
async function waitForUsageLimitSignal(readSnapshot, options = {}) {
  const timeoutMs = Number.isFinite(Number(options.timeoutMs)) ? Number(options.timeoutMs) : 12_000;
  const intervalMs = Number.isFinite(Number(options.intervalMs)) ? Number(options.intervalMs) : 100;
  const stopWhen = typeof options.stopWhen === "function" ? options.stopWhen : null;
  const isMatch = typeof options.isMatch === "function" ? options.isMatch : Boolean;

  return waitForPredicate(readSnapshot, {
    timeoutMs,
    intervalMs,
    stopWhen,
    predicate: isMatch,
