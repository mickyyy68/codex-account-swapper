...
const USAGE_LIMIT_MESSAGE_RE = /\b(?:usage limit|rate[- ]?limit(?:ed|ing)?|quota (?:exceeded|reached|limit)|(?:\d+[- ]?hour|weekly|daily|monthly) limit|reached your (?:usage|rate|quota|monthly|weekly|daily|\d+[- ]?hour) limit)\b/i;

function isUsageLimitErrorCode(code) {
  return typeof code === "string" && USAGE_LIMIT_ERROR_CODES.has(code.trim());
}

function hasUsageLimitMessageText(message) {
  return USAGE_LIMIT_MESSAGE_RE.test(String(message || ""));
}

// Narrow TUI-only patterns. Codex surfaces certain usage-limit failures
// (e.g. its background "remote compact task") in the rendered terminal
// stream without writing them to the session JSONL, so the session-state
// detector cannot see them. This regex matches the specific compact-task
// usage-limit line, tolerating Codex's line-wrapping between the two
// anchors (up to ~400 chars of wrapped body).
const TUI_USAGE_LIMIT_PATTERNS = Object.freeze([
  /Error running remote compact task[\s\S]{0,400}?(?:usage limit|rate[- ]?limit(?:ed|ing)?|quota (?:exceeded|reached|limit))/i,
]);

function hasTuiUsageLimitError(text) {
  const value = String(text || "");
  if (!value) {
    return false;
  }
  for (const pattern of TUI_USAGE_LIMIT_PATTERNS) {
...
module.exports = {
  ...
  USAGE_LIMIT_MESSAGE_RE,
  TUI_USAGE_LIMIT_PATTERNS,
};
